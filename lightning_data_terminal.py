...
class SameDateSampler(BatchSampler):
    ...
    """
    def __init__(self, data_source, batch_size, drop_last:bool = False, random_date_per_epoch:bool=True):
    ...
    def __iter__(self):
        if self.random_entry:
            for i in range(0, len(self.chosen_class), self.batch_size):
                target = self.chosen_class[i:i+self.batch_size]
...
class MultiCamDailyCowsDataModule(LightningDataModule):
    def __init__(self, name:str = "MultiCamDailyCows2023", root:int = 1, date_pin:list = [3, 1, 3], batch_size:int = 16, num_workers:int = 16, balanced_sample:bool = True):
    ...
    def retrieve_dataset(self, type):
        ...
        return dataset_prefix(root=self.root, date_pin=self.date_pin, type=type, transform=self.transform)
    ...
    def train_dataloader(self):
        ...
        same_date_sampler = SameDateSampler(self.train_set, batch_size=self.batch_size, drop_last=False, random_date_per_epoch=True)
        return DataLoader(self.train_set,
    ...
    def test_dataloader(self):
        test_sampler = ImbalancedDatasetSampler(self.test_set) if self.__apply_balanced_sample else None
